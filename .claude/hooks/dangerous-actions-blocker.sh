...
if [[ "$TOOL_NAME" == "Bash" ]]; then
    ...
    if echo "$SCAN" | grep -qE 'rm\s+(-[a-zA-Z]+\s+)*-?[a-zA-Z]*[rR][a-zA-Z]*\s+[^;&|]*(\.venv|packages/?\s*$|\.git(/|\s|$)|\.worktrees)'; then
        block "Recursive delete of a repo-critical path (.venv, packages/, .git, .worktrees)." \
    ...
    # --- warn (non-blocking) ----------------------------------------------
    if echo "$SCAN" | grep -qE 'git\s+(merge|pull)\b' && [[ -d .worktrees ]]; then
