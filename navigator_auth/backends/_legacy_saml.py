...
from ..conf import SAML_PATH, SAML_SETTINGS, SAML_MAPPING
...
class SAMLAuth(ExternalAuth):
    ...
    async def authenticate(self, request: web.Request):
        ...
        # 2. Get Redirect URL
        try:
            return_to = self.redirect_uri.format(domain=self.get_domain(request), service=self._service_name)
        ...
        if not errors:
            if auth.is_authenticated():
                ...
                try:
                    ...
                    data = await self.validate_user_info(request, uid, userdata, token=name_id)
                    ...
                    if "RelayState" in req["post_data"]:
                        ...
                        if internal_redirect == self.redirect_uri.format(
                            domain=self.get_domain(request), service=self._service_name
                        ):
                    ...
                    return self.home_redirect(request, token=token, token_type="Bearer", uri=internal_redirect)

                except Exception as err:
                    self.logger.exception(f"SAML: Error getting User information: {err}")
                    return self.failed_redirect(
                        request, error="ERROR_USER_INFO", message=f"Error processing user info: {err}"
                    )
            else:
                return self.failed_redirect(
                    request, error="NOT_AUTHENTICATED", message="User not authenticated in SAML response"
                )
        else:
            self.logger.error(f"SAML Errors: {errors}")
            reason = auth.get_last_error_reason()
            return self.failed_redirect(request, error="SAML_ERROR", message=f"SAML Error: {errors}, Reason: {reason}")
