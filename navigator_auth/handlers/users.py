...
class UserHandler(BaseView):
        ...
        if args:
            ...
            async with await db.acquire() as conn:
                ...
                try:
                    ...
                    headers = {"x-error": f"User was not Found: {args!r}"}
            ...
            self.error(reason="Invalid or missing User Data to Patch", status=403)
    ...
    async def delete(self):
        ...
        try:
            ...
            self.error(reason="Invalid User Data", status=403)
        ## getting first the id from params or data:
        args = {}
        try:
            userid = data["user_id"]
        ...
        if args:
            async with await db.acquire() as conn:
                ...
                result = await self.model.get(**args)
                if not result:
                    self.error(reason="User was Not Found", status=204)
                # Delete them this Client
                data = await result.delete()
                return self.json_response(content=data, status=202)
        else:
            self.error(reason="Cannot Delete an missing User ID", status=404)
    ...
    async def put(self):
        ...
        try:
            ...
            return self.error(reason="Invalid User Data", status=403)
        ...
        try:
            ...
            return self.error(exception=error, status=400)
