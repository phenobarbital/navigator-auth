...
class BasicAuth(BaseAuthBackend):
    ...
    async def validate_user(self, login: str = None, password: str = None):
        ...
        try:
            ...
            raise InvalidAuth(f"Invalid User Information: {ex.payload}") from ex
        ...
        try:
            ...
            raise InvalidAuth(f"Missing Password attribute on User {login}") from ex
        ...
        try:
            if self._idp.check_password(pwd, password):
                ...
                raise FailedAuth(f"User {login}: Invalid Credentials")
            ...
            raise InvalidAuth(f"Unknown Password Error: {err}") from err
    ...
    async def authenticate(self, request):
        ...
        if not pwd and not user:
            raise InvalidAuth("Basic Auth: Missing Credentials", status=401)
        else:
            # making validations
            try:
    ...
    async def check_credentials(self, request):
        ...
        try:
            ...
            self.logger.error(f"User Not Found: {err}")
            raise self.Unauthorized(reason="User Not Found") from err
        except Exception as err:
        ...
        if not userdata:
            raise InvalidAuth("Invalid User Information")
        uid = user[self.userid_attribute]
        usr = await self.create_user(userdata[AUTH_SESSION_OBJECT])
        usr.id = uid
        usr.set(self.username_attribute, username)
