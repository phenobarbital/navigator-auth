...
def test_policy_creation():
    assert policy.name == 'only_for_jesus'
    assert policy.effect == PolicyEffect.ALLOW
    assert policy.subject == ['jlara@trocglobal.com']
...
def test_policy_evaluate():
    ...
    if (dummy_environment['hour'] in policy.environment['hour'] and
        dummy_environment['day_of_week'] in policy.environment['day_of_week']):
