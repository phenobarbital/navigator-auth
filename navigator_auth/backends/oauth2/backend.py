...
class Oauth2Provider(BaseAuthBackend):
    ...
    def get_domain(self, request: web.Request) -> str:
        ...
        self.logger.debug(f"OAUTH2 DOMAIN URL: {domain_url}")
        return domain_url
    ...
    async def authorize(self, request: web.Request):
        ...
        payload = {**data, "scope": data.get("scope", "default"), "client_name": client.client_name}
        url = location.with_query(**payload)
        self.redirect(url, location=True)
    ...
    async def consent(self, request: web.Request):
        ...
        if request.method == "GET":
            ...
            if action == "approve":
                ...
                payload = {"code": auth_code, "state": data.get("state")}
        ...
        if request.method == "POST":
            if ctype in (
                ...
                try:
                    ...
                    self.logger.error(f"Oauth2: Error getting JSON data from request: {err}")
    ...
    async def auth_login(self, request: web.Request):
        ...
        if request.method == "GET":
            ...
            if not password and not username:
                raise web.HTTPBadRequest(reason="Auth: Invalid Credentials")
            try:
    ...
    async def token_request(self, request):
        ...
        if grant_type == "authorization_code":
            ...
            access_token, refresh_token_str, exp, scheme = self._idp.create_token(token_payload)
            ...
            # App-Only Token
            token_payload = {"user_id": client.user.user_id, "client_id": client_id, "aud": "app"}  # On behalf of user

            # TODO: Set duration to 2 hours
            access_token, refresh_token_str, exp, scheme = self._idp.create_token(token_payload)

            response = {"access_token": access_token, "token_type": scheme, "expires_in": exp}
            return JSONResponse(response, status=200)
            ...
            token_payload = {"user_id": user.user_id, "client_id": client_id, "scope": rt.scope}
